...
#       -top_level_dir：这个是顶层目录的名称，一般默认等于 None 就行了
    ...
    discover=unittest.defaultTestLoader.discover(case_dir,pattern='*.py',top_level_dir=False)
...
#     runner.run(all_case())
    ...
    # title 测试报告主题
    # description：测试报告的描述
    report_path=os.path.join(os.getcwd(), "report\\result.html")
    print report_path
    fp=open(report_path,"wb")
    runner=HTMLTestRunner.HTMLTestRunner(stream=fp,
                                         title=u"这是我的接口自动化测试报告",
                                         description=u"用例执行情况")
#     runner = HTMLTestRunner(title="带截图的测试报告", description="小试牛刀", stream=fp, verbosity=2, retry=0, save_last_try=True)
    runner.run(all_case())
    fp.close()